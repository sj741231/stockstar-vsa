...
#edit by shiin 增加手机发送短信通知模块
from admanage.smsaduser import sms_aduser
    ...
    for i in range(1, table.nrows):
        ...
        if GroupReleation.objects.filter(adtree=t[1]):
            ...
            if account:
                ...
                #edit by shijin 
                     #添加手机号,判断文件是否有手机号列，判断手机号是否为空。有手机号写入MODELS SmsAduser
                if len(t) == 5:
                    if t[4]:
                        #SmsAdUser.objects.create(aduser = t[0], adaccount = account, department = t[1], aduserfile = xls_file, mobile = str(int(t[4])), sendstatus = False, detail='')
                        smsuser={'aduser': t[0],'mobile': str(int(t[4])),'adaccount': account,'aduserfile': xls_file,'department': t[1],'sendstatus': False ,'detail':''}
